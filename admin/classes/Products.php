...
class Products
{
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce-app-h/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						...
						return ['status'=> 303, 'message'=> 'Failed To Product Adding '];
					}
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce-app-h/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						...
						return ['status'=> 303, 'message'=> 'Failed To Product Modifying '];
					}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			...
			if ($q) {
				...
				return ['status'=> 303, 'message'=> 'Failed to adding New Category'];
			}
		}
	}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			if ($q) {
				...
				return ['status'=> 202, 'message'=> 'Please Remove Product First'];
			}
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				...
				return ['status'=> 202, 'message'=> 'Failed to update Category'];
			}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Sub Category already exists'];
		}else{
			$q = $this->con->query("INSERT INTO brands (brand_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'New Sub Category added Successfully'];
			}else{
				return ['status'=> 303, 'message'=> 'Failed to add New Sub Category'];
			}
		}
	}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Sub Category removed'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to Remove Sub Category'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Invalid Sub Category id'];
		}
	...
	public function updateBrand($post = null){
		...
		if (!empty($brand_id) && !empty($e_brand_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Sub Category updated'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to update Sub Category'];
			}
